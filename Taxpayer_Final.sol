...
/// @notice Improved Taxpayer contract with bidirectional marriage/divorce
contract Taxpayer {
 ...
 /// @notice Public function to marry with bidirectional update
 function marry(address new_spouse) public {
  ...
  require(Taxpayer(new_spouse).isContract(), "Spouse must be valid Taxpayer contract");
  
  // Write marriage for this taxpayer
  _writeMarriage(new_spouse);
  
  // Automatically update spouse's record
  Taxpayer(new_spouse)._writeMarriage(address(this));
 }
 ...
 /// @notice Public function to divorce with bidirectional update
 ...
 function setTaxAllowance(uint ta) public {
    require(Taxpayer(msg.sender).isContract() || Lottery(msg.sender).isContract(), "Unauthorized");
    tax_allowance = ta;
 }
